...
# %% New: Class for unlabeled Dataset
class UnlabeledDataset(Dataset):
    semi_transformations = {0: 0, 1: 90, 2: 180, 3: 270}
    ...
    def __getitem__(self, idx):
        ...
        # get a random semi transformation
        transformation_class_label = random.randint(0, len(self.semi_transformations)-1)
        # apply randomly selected transformation
        angle = self.semi_transformations[transformation_class_label]
        data = transform_super(img)
        data = transforms.functional.rotate(img=data, angle=angle)
        return data, transformation_class_label

# %% New: Dataset for unlabeled data
...
model = SemiNet(n_super_classes=2, n_selfsuper_classes=4)
model.train()
...
# %%
# 89% for supervised learning (baseline for semi-supervised)